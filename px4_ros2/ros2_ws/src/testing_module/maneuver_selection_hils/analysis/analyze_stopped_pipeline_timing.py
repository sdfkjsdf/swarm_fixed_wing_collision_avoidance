...
def analyze(text, low=0, high=None):
    ...
    report = dict(vehicle=header[1], stage_timing=base,
        ...
        source_to_callback_wall=stats([(r[3]-r[0]*1000)/1e6 for r in callbacks]),
    ...
    for stage, name in [(1,'trajectory_refresh'),(2,'combination_selection')]:
        completed = [r for r in stages if r[0]==stage and inside(r[1])
                     and r[8] and (stage != 1 or r[7])]
    ...
    report['limits'] = [
        ...
        'Source-to-callback uses system clocks and includes source pipeline, network and executor waiting; not pure network delay.',
