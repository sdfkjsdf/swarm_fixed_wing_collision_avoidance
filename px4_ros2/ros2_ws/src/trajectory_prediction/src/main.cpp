/* ════════════════════════════════════════════════════════════════════
   ...
   부팅 흐름:
     ...
     [4] VtolGuidanceExecutor 생성 (Preflight ↔ Replay 인계)
...
#include <trajectory_prediction/TrajectoryReplayMode.hpp>
#include <trajectory_prediction/VtolGuidanceExecutor.hpp>
...
#include <collision_avoidance/collision_estimation/trajectory_prediction/TrajectoryPredict.hpp>
...
using collision_avoidance::collision_estimation::PredictState;
using collision_avoidance::collision_estimation::PredictInput;
using collision_avoidance::collision_estimation::PredictParams;
using collision_avoidance::collision_estimation::TrajectoryPredict;
using collision_avoidance::collision_estimation::TrajectorySample;        /* ★ extractKeySamples 반환 타입 */
...
int main(int argc, char * argv[])
{
    ...
    node->declare_parameter<std::string>("sequence_file",
        "/home/leedonghyuck/ros2_ws/src/trajectory_prediction/config/setpoint_sequence.yaml");
    ...
    auto executor = std::make_shared<VtolGuidanceExecutor>(*preflight, *replay);
    ...
    auto predict_timer = node->create_wall_timer(
        ...
        [predictor, logger, sequencer_for_predict, predicted_traj, dt, k_g, alt_offset,
         key_samples_pub]() {
            ...
            namespace ce = collision_avoidance::collision_estimation;
