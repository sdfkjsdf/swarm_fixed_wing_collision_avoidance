...
using collision_avoidance::collision_estimation::PoseVel;
using collision_avoidance::collision_estimation::TrajectorySample;
    ...
    const MeasuredSnapshot &    m_at_pred)
{
    ...
    auto state_to_ned = [](const PredictState & s) -> collision_avoidance::collision_estimation::Vec3 {
        /* NED 변환: extractAt 와 동일 — pd = -h. 단 본 메서드는 위치만 추출. */
        return { static_cast<float>(s.p_n), static_cast<float>(s.p_e), static_cast<float>(-s.h) };
    };
    auto state_to_vel_ned = [](const PredictState & s) -> collision_avoidance::collision_estimation::Vec3 {
