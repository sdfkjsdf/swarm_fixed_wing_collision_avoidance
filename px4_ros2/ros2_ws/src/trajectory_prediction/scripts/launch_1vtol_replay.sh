...
#   - ros2 + ros2_ws 빌드 완료 (trajectory_prediction)
# ════════════════════════════════════════════════════════════════════
...
if [[ "${LAUNCH_NODE}" == "1" ]]; then
    ...
    echo "  ros2 run trajectory_prediction trajectory_replay_node --ros-args \\"
    echo "      -p topic_namespace_prefix:=/px4_${INSTANCE} -p vehicle_ID:=${INSTANCE} \\"
    echo "      --params-file src/trajectory_prediction/config/replay_params.yaml \\"
    echo "      --params-file src/trajectory_prediction/config/airframe_spec.yaml"
    echo ""
