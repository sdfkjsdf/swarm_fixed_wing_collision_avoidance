...
bool TrajectoryUncertainty::propagateAlongMeanWithCommandDelay(
    const TrajectoryPredict & predictor,
    const PredictionMeanTrajectory & mean_trajectory,
    const PredictStateCovariance & initial_covariance,
...
{
    PredictStateCovariance covariance = initial_covariance;
    if (!(dt > 0.0) || !std::isfinite(command_delay_s)
        || command_delay_s < 0.0 || !finiteState(mean_trajectory.front())
        || !covarianceIsFiniteAndPsd(covariance)) {
        return false;
    }
    ...
    for (std::size_t index = 0; index < kTrajectoryIntervalCount; ++index) {
        ...
        const double interval_start_s = static_cast<double>(index) * dt;
        const double current_dt = command_delay_s > interval_start_s
            ? std::min(dt, command_delay_s - interval_start_s)
            : 0.0;
        PredictState linearization_state = current_mean;
        if (current_dt > 0.0) {
            const PredictState predicted_next = predictor.stepRK4(
                linearization_state, current_input, current_dt);
            if (!propagateCovarianceOneStep(
                    predictor,
                    current_input,
                    current_dt,
            ...
            linearization_state = predicted_next;
        }

        const double candidate_dt = dt - current_dt;
        if (candidate_dt > 0.0) {
            const PredictState predicted_next = predictor.stepRK4(
                linearization_state, candidate_input, candidate_dt);
            if (!propagateCovarianceOneStep(
                    predictor,
                    candidate_input,
                    candidate_dt,
