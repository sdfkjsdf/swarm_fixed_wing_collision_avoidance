...
namespace StateType
{
    ...
    struct FwSetpointOutput_rt2mt {
        ...
        float height_setpoint      = NAN;  /* [m] ★ 신규 — FlockingGuidance 가 받은 *목표 고도* (NED z 의 negation = altitude up).
                                                  P 제어로 height_rate 도출 후에도 원본 정보 보존 →
                                                  trajectory_prediction 의 PD 적분에서 PredictInput.h_cmd 로 사용.
                                                  NAN = 미설정 (cruise fallback 등) */
