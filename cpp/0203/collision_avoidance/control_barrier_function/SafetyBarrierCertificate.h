...

{
  ...
  void calculate_h_x_i_gradient_h_x_i( const matrix::Vector<double,11>& state_i,const TsMessage_t& other_information);



  void calculate_grad_pij_x();

  void calculate_grad_vij_x(const double speed,
                            const double c_p,
                            const double s_p,
                            const double c_y,
                            const double s_y);
  ...
  private:
    double m_r_p=10.0;
    double m_critical_distance=30.0;
    /* h(x)에 대한 편미분 관련 내용*/
    double m_h_x=0.0;
    ...
    matrix::Matrix<double,3,11> m_grad_pij_x{};
    matrix::Matrix<double,3,11> m_grad_vij_x{};
