...
void DynamicsLinearization::calculate_zoh_matrix(const matrix::Vector<double, 11 >& state)

{
    calculate_fx_gx_Jf_x(state);
    calculate_C(state, m_Jf_x);
    calculate_Ad_Bd();
    calculate_BdB();
    calculate_BdC();
...
matrix::Vector<double,11> DynamicsLinearization::propagtion_zoh(const matrix::Vector<double,11>& state, const matrix::Vector<double,4>& control_input)

{
    calculate_zoh_matrix(state);
    return m_Ad * state + m_BdB * control_input + m_BdC;
