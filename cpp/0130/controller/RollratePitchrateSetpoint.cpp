...
RollratePitchrateSetpoint::RollratePitchrateSetpoint(const GainParameter_t& gain)
                                                                                 ...
                                                                                 const double& gradient_fev_pitch)
 {
   ...
   const double alpha_upper = ( (gradient_lyaponove*pitchrate_sp_rar)+( lyaponove_k*lyaponove_pitch) ) /( (gradient_lyaponove*gradient_lyaponove) + tol);

   const double alpha=std::max(0.0, alpha_upper);

   m_output_rate_of_rollpitch(1)=pitchrate_sp_rar- (alpha*gradient_lyaponove);



   /*Fev 적용 */

//   bool safe_inside = (lyaponove_pitch <= tol);  // g ≤ 0 (tolerance 포함)
//   bool boundary_safe_dir = (gradient_lyaponove * pitchrate_sp_rar <= 0.0);

//   bool pass_cond = safe_inside || boundary_safe_dir;
