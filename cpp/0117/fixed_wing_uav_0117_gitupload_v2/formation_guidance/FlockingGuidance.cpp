...
{ 
  ...
  matrix::Vector<double,2> relative_velocity= matrix::Vector<double,2>(StateInformation::get_state_other_state(total_message[i]).slice<2,1>(2,0))-  self_velocity ;  
...
double roll_sp_rar = std::atan(ground_speed * yawrate_setpoint_rar/g);


double roll_sp = max_roll * std::tanh(roll_sp_rar / max_roll);
double yawrate_setpoint_after_smoothh  = (g * tan(roll_sp)) / ground_speed;


m_roll_yawrate_ground_acc(0)=roll_sp;
m_roll_yawrate_ground_acc(1)=yawrate_setpoint_after_smoothh;
m_roll_yawrate_ground_acc(2)=ground_acclertion_setpoint;
