...
int main() {
    ...
    for (int i = 0; i < total_vehicle_num; i++) {
        agents.emplace_back(i, g_FixedwingSpec, g_Flockingparameter, 
                           g_LeaderFollowerParameter, g_GainParameter, g_NdiParameter);
    }
    ...
    std::uniform_real_distribution<> speed_dist(g_FixedwingSpec.min_speed + 5, 
                                                g_FixedwingSpec.max_speed - 5);
    std::uniform_real_distribution<> at_dist(g_FixedwingSpec.at_min, g_FixedwingSpec.at_max);

    std::vector<matrix::Vector<double, 11>> states(total_vehicle_num);
    
    for (int i = 0; i < total_vehicle_num; i++) {
        states[i].setZero();
        states[i](0) = north_dist(gen);
        states[i](1) = east_dist(gen);
        states[i](2) = down_dist(gen);
        states[i](5) = yaw_dist(gen);
        states[i](6) = speed_dist(gen);
        states[i](10) = at_dist(gen);
        
        agents[i].receive_state_from_sim(states[i]);
        
        std::cout << "Agent " << i << " initialized: "
                  << "N=" << states[i](0) 
                  << " E=" << states[i](1)
                  << " D=" << states[i](2) << std::endl;
    }
