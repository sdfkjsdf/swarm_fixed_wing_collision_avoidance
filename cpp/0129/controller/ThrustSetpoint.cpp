...
{   
    ...
    /*stall 방지용 마진을 구하기*/
    const double stall_margin_abs = 0.5;  // [m/s]
    const double speed_lower = std::min(stall_speed + stall_margin_abs, max_speed);

    const double speed_setpoint_before_fev=( (1/k_v)*dot_speed_setpoint )+speed;
    const double speed_setpoint_after_fev=std::clamp(speed_setpoint_before_fev,speed_lower,max_speed);
